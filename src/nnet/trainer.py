...
from models.gracr import GRACR
...
class Trainer:
    ...
    def init_model(self):
        model_0 = GRACR(self.params, self.loader.pre_embeds,
    ...
    def eval_epoch(self, final=False, save_predictions=False):
        ...
        if save_predictions:
            ...
            for item in test_result:
                ...
                if(test_info[item[-2]][item[-1]]['pmid'] == '0'):
                    print("==============================================================>")
                    print([(item[-3], item[1])])
                    print(test_info[item[-2]][item[-1]])
    ...
    def convert_batch(self, batch, istrain=False, save=False):
        ...
        batch_ = [{k: v for k, v in b.items() if (k != 'info' and k != 'text' and k != 'rgcn_adjacency' and k != 'ent_adjacency')} for b in batch]
        # batch_ = [{k: v for k, v in b.items() if (k != 'info' and k != 'text' and k != 'ent_adjacency')} for b in batch]
        ...
        new_batch['ent_adjacency'] = convert_3dsparse_to_4dsparse([b['ent_adjacency'] for b in batch]).to(self.device)
        # print(new_batch['dep_adj'])
        new_batch['ners'] = converted_batch['ners'][converted_batch['ners'] != -1].long().contiguous()
        # new_batch['entity2mention'] = [b['entity2mention']]
        if save:
